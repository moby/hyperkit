...
func (pfs packageFS) Open(name string) (http.File, error) {
	if name != "/" {
		return nil, &os.PathError{Op: "open", Path: name, Err: os.ErrNotExist}
	}
	p, err := build.Import(pfs.importPath, "", build.FindOnly)
	if err != nil {
		return nil, &os.PathError{Op: `"go/build".Import`, Path: pfs.importPath, Err: err}
	}
